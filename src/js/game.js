...
class GravityManGame {
  constructor() {
    ...
    this.images = { splash: this.loadImage('grav1tyman-splash.png'), end: this.loadImage('endscreen.png'), player: null };
  ...
  drawPlaying(){ const c=this.ctx, lvl=this.currentLevel; c.fillStyle=this.colors.bg; c.fillRect(0,0,this.canvas.width,this.canvas.height); // platforms
    c.fillStyle=this.colors.platform; for(const p of lvl.platforms) c.fillRect(p.x,p.y,p.width,p.height); // hazards (white but dashed outline to differentiate)
    c.fillStyle=this.colors.hazard; for(const h of lvl.hazards) c.strokeStyle=this.colors.fg, c.strokeRect(h.x,h.y,h.width,h.height); // draw enemies
    c.fillStyle=this.colors.enemy; for(const e of (lvl.enemies||[])) c.fillRect(e.x,e.y,e.w,e.h); // draw parts
    c.fillStyle=this.colors.part; for(const part of lvl.parts) if(!part.collected) c.fillRect(part.x,part.y,part.width,part.height);
    // goal
    ...
    // HUD
    c.fillStyle=this.colors.fg; c.font='8px monospace'; c.fillText(`LEVEL ${this.levelIndex+1}/5`, 4, 10); c.fillText(`PARTS ${this.collectedParts}/${this.totalParts}`, 120, 10);
  }
  drawEnd(){ this.ctx.fillStyle=this.colors.bg; this.ctx.fillRect(0,0,this.canvas.width,this.canvas.height); if(this.images.end?.complete) this.ctx.drawImage(this.images.end,0,0,240,282); this.ctx.fillStyle=this.colors.fg; this.ctx.font='10px monospace'; this.ctx.textAlign='center'; this.ctx.fillText('Raumschiff repariert! Spiel gewonnen.', this.canvas.width/2, this.canvas.height-18); this.ctx.textAlign='left'; }

  loop(ts){ const dt=Math.min(32, ts-(this.lastTime||ts)); this.lastTime=ts; if(this.state==='playing') this.update(dt); switch(this.state){ case 'splash': this.drawSplash(); break; case 'playing': this.drawPlaying(); break; case 'end': this.drawEnd(); break; } requestAnimationFrame(this.loop.bind(this)); }
}
