...
class LevelManager{
  ...
  make(){
    ...
    const L=[]; for(let i=0;i<5;i++){ const l=mk();
      // Procedural platform rows increase with level index i
      ...
      // Vertical pillars
      for(let p=0;p<2+i;p++){ const vx=(rnd(40,this.w-60)|0); l.walls.push({x:vx,y:28,width:8,height:(rnd(40,this.h-80)|0)}); }
      ...
      // Moving enemies (Daleks) affected by gravity axis in their patrol axis
      const ec=2+i; for(let e=0;e<ec;e++){ const hor=e%2===0; const ex=clamp((rnd(40,this.w-60)|0),40,this.w-60); const ey=clamp((rnd(60,this.h-80)|0),60,this.h-80); l.enemies.push({x:ex,y:ey,w:14,h:16,home:{x:ex,y:ey},hor,dir:1,range:26+(i*6),v:0.7+(i*0.1)}); }
      // Start position changes per level
      l.start = { x: 18+(i*8), y: 20+(i*6) };
      // Exit per level is in far corner initially
  ...
  render(ctx, sprites){
    ...
    // Exit gate (only colored glow when open; texture itself grayscale)
    if(l.exit){ const e=l.exit; if(sprites.exit.loaded){ ctx.save(); if(e.open){ for(let g=0;g<3;g++){ ctx.fillStyle=`rgba(220,255,220,${0.22-g*0.06})`; ctx.fillRect(e.x-2-g,e.y-2-g,e.w+4+2*g,e.h+4+2*g);} }
      ...
      ctx.restore(); } else { ctx.fillStyle='#BDBDBD'; ctx.fillRect(this.w-34,this.h-34,16,16); }
    // BW walls
...
class Game{
  ...
  bind(){
    ...
    // Swipe gravity
    let t0=null; this.c.addEventListener('touchstart',e=>{const t=e.changedTouches[0]; t0={x:t.clientX,y:t.clientY};},{passive:true}); this.c.addEventListener('touchend',e=>{ if(!t0) return; const t=e.changedTouches[0], dx=t.clientX-t0.x, dy=t.clientY-t0.y; if(Math.max(Math.abs(dx),Math.abs(dy))>12){ if(Math.abs(dx)>Math.abs(dy)) setG(dx>0?'right':'left'); else setG(dy>0?'down':'up'); } t0=null;},{passive:true});
    document.addEventListener('keydown',(e)=>{ switch(e.code){ case 'Space': e.preventDefault(); if(this.mode==='splash') this.start(); else this.paused=!this.paused; break; case 'KeyA': if(this.mode==='playing') this.p.boost(); break; case 'ArrowUp': setG('up'); break; case 'ArrowDown': setG('down'); break; case 'ArrowLeft': setG('left'); break; case 'ArrowRight': setG('right'); break; } },{passive:false});
    if('DeviceOrientationEvent' in window){ window.addEventListener('deviceorientation',ev=>{ const ax=Math.abs(ev.gamma||0), ay=Math.abs(ev.beta||0); if(ax>ay) setG((ev.gamma||0)>8?'right':(ev.gamma||0)<-8?'left':this.p.g||'down'); else setG((ev.beta||0)>8?'down':(ev.beta||0)<-8?'up':this.p.g||'down'); }); }
    // Rabbit SDK (optional)
    const sdk=window.creations||window.r1||window.rabbit||null; if(sdk?.on){ try{ sdk.on('ptt',p=>{ if(!p) return; if(this.mode==='splash') this.start(); else this.paused=!this.paused; }); sdk.on('panel',v=>this.panelVisible=!!v); sdk.on('orientation',({alpha,beta,gamma})=>{ const ax=Math.abs(gamma||0), ay=Math.abs(beta||0); if(ax>ay) setG((gamma||0)>8?'right':(gamma||0)<-8?'left':this.p.g||'down'); else setG((beta||0)>8?'down':(beta||0)<-8?'up':this.p.g||'down'); }); }catch(_){}}
  ...
  update(){ if(this.mode!=='playing'||this.paused) return; const l=this.levels.cur(); this.p.update(l);
    ...
    if(l.exit.open && this.p.coll({x:l.exit.x,y:l.exit.y,w:l.exit.w,h:l.exit.h})){
      this.levelClears++; if(this.levels.next()){ const nl=this.levels.cur(); const s=nl.start; this.p.x=s.x; this.p.y=s.y; this.p.vx=this.p.vy=0; this.p.setGravity('down'); this.collected=0; nl.items.forEach(i=>i.collected=false); nl.exit.open=false; }
